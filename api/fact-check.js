...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!apiKey || apiKey.trim() === '' || apiKey.trim() === 'your_gemini_api_key_here') {
      console.warn('[Gemini Warning] GEMINI_API_KEY is missing or unconfigured.');
      return res.status(400).json({
        error: 'AI service is not configured. Please set a valid GEMINI_API_KEY environment variable in your Vercel project settings to enable live Gemini fact-checking.'
      });
    }
    ...
    for (const modelName of modelsToTry) {
      try {
        console.log(`[Gemini Serverless] Calling Google Gemini API model: ${modelName}...`);
    ...
    console.error('Vercel serverless function internal error:', error);
